...
class Orchestrator:
    ...
    def _generate_robco_ini(self):
        ...
        Robco Patcher INI ファイルを生成する。
        - robco_ammo_patch.ini: 弾薬分類データ
        - robco_ll_patch.ini: レベルドリスト統合データ（必要な入力がある場合のみ）
        """
        ...
        try:
            ...
            if not strategy_file.is_file():
                logging.error(f"[RobcoINI] 必須ファイル未存在: {strategy_file}")
                return False
            ...
            ammo_count = 0
            for ammo_formid, ammo_data in ammo_classification.items():
                ammo_patch_lines.append(f"\n[Ammo.{ammo_formid}]")
                ammo_patch_lines.append(f"category = {ammo_data.get('Category', 'Unknown')}")
                ammo_patch_lines.append(f"power = {ammo_data.get('Power', 0)}")
                ammo_count += 1
            
            ammo_patch_path = robco_patcher_dir / 'robco_ammo_patch.ini'
            ammo_patch_path.write_text("\n".join(ammo_patch_lines), encoding='utf-8')
            logging.info(f"[RobcoINI] robco_ammo_patch.ini 生成成功 (弾薬数={ammo_count})")

            # ========== robco_ll_patch.ini 生成（条件付き）==========
            if not weapon_data_file.is_file():
                logging.warning(f"[RobcoINI] weapon_ammo_map.json が存在しないため robco_ll_patch.ini をスキップ")
                return True
            ...
            try:
                ...
                logging.warning(f"[RobcoINI] robco_ll_patch.ini 生成をスキップ")
                return True

            # ammo_map からマッピング読込（ammo_map.json 優先、ammo_map.ini フォールバック）
            ammo_map_dict = {}
            ammo_map_json_file = output_dir / 'ammo_map.json'
            
            if ammo_map_json_file.is_file():
                # ammo_map.json を優先使用
                try:
                    ammo_map_data = json.loads(self._read_text_utf8_fallback(ammo_map_json_file))
                    ammo_map_dict = {k.lower(): v.lower() for k, v in ammo_map_data.items()}
                    logging.info(f"[RobcoINI] ammo_map.json から {len(ammo_map_dict)} 件のマッピング読込")
                except Exception as e:
                    logging.warning(f"[RobcoINI] ammo_map.json 読込失敗: {e}, ammo_map.ini にフォールバック")
            ...
            elif ammo_map_file.is_file():
                # ammo_map.json が無い場合は ammo_map.ini を使用
                parser = configparser.ConfigParser()
                parser.read(ammo_map_file, encoding='utf-8')
                if parser.has_section('UnmappedAmmo'):
                    ammo_map_dict = {k.lower(): v.lower() for k, v in parser.items('UnmappedAmmo')}
                    logging.info(f"[RobcoINI] ammo_map.ini から {len(ammo_map_dict)} 件のマッピング読込")
            ...
            for weapon in weapon_data:
                ...
                for faction, lli_name in faction_leveled_lists.items():
                    # SuperMutants を除外
                    if faction == "SuperMutants":
                        continue
                    
                    faction_alloc = allocation_matrix.get(faction, {})
                    weight = faction_alloc.get(ammo_info["Category"])
                    if weight and weight > 0:
                        # invertedChance = 100 - weight (重みをパーセントとして解釈)
                        inverted_chance = 100 - int(weight * 100)
                        leveled_entries.append(f"{lli_name}@{ammo_info['Category']}:{inverted_chance}")
                        used_leveled_lists.add(lli_name)

                if not leveled_entries:
                    continue

                safe_id = editor_id.replace(' ', '_')
                ll_patch_lines.append(f"\n[Weapon.{safe_id}]")
                ll_patch_lines.append(f"name = {full_name}")
                ll_patch_lines.append(f"leveled_lists = {', '.join(leveled_entries)}")
                processed += 1

            ll_patch_path = robco_patcher_dir / 'robco_ll_patch.ini'
            ll_patch_path.write_text("\n".join(ll_patch_lines), encoding='utf-8')
            logging.info(f"[RobcoINI] robco_ll_patch.ini 生成成功 (武器処理数={processed}, 未分類スキップ={skipped_no_category})")
            logging.info(f"[RobcoINI] 使用レベルドリスト数: {len(used_leveled_lists)}")
